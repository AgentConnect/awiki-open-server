...
> **采用前必须知道：** 当前版本是面向小规模部署的单节点 MVP；消息不是端到端加密；不包含生产短信/邮件验证、高可用、离线推送、大群 fanout、复杂群治理、federation relay、计费或托管 Agent 编排。不要把它直接当作敏感通信或大规模生产平台。
...
- 在单节点环境中实验身份、消息、Community Group 托管/互通、附件和 Markdown 站点；
...
- 需要生产级手机号/邮箱/企业身份提供方；
- 需要大群、高并发 fanout、复杂治理、计费、多租户托管或托管 Agent Runtime；
- 需要 federation relay/peer-route mesh、多区域复制或远程对象 relay。
...
| Group | 小规模群创建与管理、open-join/admin-add、本地与跨域消息、history、sync、read state 和 realtime projection |
...
| 大群与复杂治理 | 无高并发 fanout、自定义角色、审批流、归档、所有权迁移或分布式 Group Host |
| Federation 基础设施 | 跨域群采用 DID discovery 直连和本地 durable outbox；无 peer-route mesh、relay、多区域复制或 remote object relay |
...
仓库提供重复可执行的 Rust CLI Gate，覆盖本地注册、Direct、两个 Host 方向的 Community Group lifecycle、People 和 Site 等路径。不要使用 `--secure required` 连接当前 Open Server。

### AWiki Me

AWiki Me 可配置兼容租户，但需要注意：

- 基础身份、Direct、Community Group 流程和附件能力需要按版本持续验证；
...
| [Community Group v1 Gate 报告](docs/community-groups-gate-report-20260716.md) | 脱敏的公网双向、receipt、retry、测试与安全证据 |
