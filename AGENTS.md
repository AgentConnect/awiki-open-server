...
`awiki-open-server` does not support end-to-end encryption. Do not add or run successful Direct E2EE, Group E2EE, secure-message, prekey, ratchet, or encrypted-attachment flows as Open Server acceptance or release gates. Tests may verify that E2EE capabilities are disabled and that related requests return `not_supported`. Cross-domain interoperability verification for this repository must use plaintext Direct and plaintext Community Group flows. E2EE tests belonging to Message Service, ANP SDK, or client repositories are outside the Open Server test scope and must be retained and run when those products are the explicit test target.
