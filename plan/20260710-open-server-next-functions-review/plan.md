...
创建时间：2026-07-10  
恢复指针：全部步骤已完成；Step 06 聚焦提交为 `6a120dc`，本文件的 hash 回填作为最终轻量台账提交。
...
| 06 | 系统测试与文档同步 | Step 05 | 串行 | 否 | coordinator | 只读 docs review 可并行 | `README.md`, `README.cn.md`, `deploy/`, `plan/`, possible `awiki-system-test/`, possible `awiki-harness/` | 最终 docs、cross-repo gate、global review evidence | [steps/06-system-test-docs-sync.md](steps/06-system-test-docs-sync.md) | 必须，如有修改 | Final local + public + docs gates | done |
...
- 最终 `git status`：Step 06 聚焦提交 `6a120dc` 后工作树 clean；hash 回填本身作为最终轻量台账提交。
- 如果本阶段修改文件：Step 06 docs/system-test commit 为 `6a120dc` (`docs: sync open server hardening gates`)。
