...
- 服务自启动状态：`enabled` / `active`。
- 公网 health、DID document、`verify-public`、公网 pytest 通过。
- Rust CLI 连接 `https://rwiki.cn` 的注册、direct send、inbox、history 通过。
- Rust CLI `rwiki.cn -> awiki.info` 通过；`awiki.info -> rwiki.cn` 仍失败于 `invalid_peer_http_signature`，归因于 `awiki.info` 公开服务 DID 文档与实际签名密钥不同步。
